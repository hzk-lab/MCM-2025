...
gold_counts = gold_medals.drop_duplicates(subset=['Year', 'Sport', 'Event', 'NOC']).groupby(['Year', 'Sport', 'Event', 'NOC']).size().reset_index(name='Gold_Medals')

# Group by Year and Sport, summing the gold medals for each country in the respective sport
gold_counts_sport = gold_counts.groupby(['Year', 'Sport', 'NOC'])['Gold_Medals'].sum().reset_index()
...
pivot_table.to_csv("updated_gold_medals.csv", index=False)

print("File 'updated_gold_medals.csv' created successfully!")
