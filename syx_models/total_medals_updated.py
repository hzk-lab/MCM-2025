...
hosts_df = pd.read_csv('summerOly_hosts.csv', encoding='ISO-8859-1')
...
hosts_df.rename(columns=lambda x: str(x).strip(), inplace=True)

# Identify the host country for 2024
host_country_2024 = hosts_df.loc[hosts_df['Year'] == 2024, 'Host'].values[0]
...
for sport in sports:
    ...
    # Boost host country's medal count
    host_boost = 0.2  # Adjust this percentage as needed (e.g., 20%)
    if host_country_2024 in predicted_medals:
        predicted_medals[host_country_2024] *= (1 + host_boost)

    # Re-normalize medal counts to match total gold medals for the sport
    total_after_boost = sum(predicted_medals.values())
    scaling_factor = total_gold_medals_2024 / total_after_boost
    for country in predicted_medals:
        predicted_medals[country] *= scaling_factor
...
# Save to CSV
total_medals_df.to_csv('total_predicted_gold_medals_2024_with_host.csv', index=False)
print("Final total predicted gold medals for 2024 (with host adjustment) saved to 'total_predicted_gold_medals_2024_with_host.csv'.")

# Visualize the total predicted medal counts
plt.figure(figsize=(14, 8))
plt.bar(total_medals_df['Country'], total_medals_df['Total_Medals'], color='gold')
plt.title("Predicted Total Gold Medals for All Sports in 2024 (With Host Boost)", fontsize=16)
